"""Strict delivery from an asynchronous application."""
...
def configured_sender():
    ...
    sender.set_source("splunk_hec_aio:async-strict-example")
...
async def send():
    ...
    try:
        results.extend(
            await sender.post_data_strict_async(
                {
                    "time": round(time.time(), 3),
                    "event": {"message": "hello from async strict delivery"},
                }
            )
        )
        results.extend(await sender.flush_strict_async())
...
if __name__ == "__main__":
    # An application with an existing event loop should await send() directly.
    asyncio.run(send())
